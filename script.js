...
const listenerEvents = ["keyup", "click"];
...
function getYear() {
	...
	const dateTimeFormatter = new Intl.DateTimeFormat(userRegion, { year: "numeric" });
...
function toggleNavigation() {
	...
	checkAttribute(mobileNavigationAttributeValue)
		? setAttribute(mobileNavigation, "aria-expanded", true)
		: setAttribute(mobileNavigation, "aria-expanded", false);
}
...
function disableNavigation(event) {
	...
	const keyEventConditions = checkNavigationCondition([conditions.conditionKey, conditions.conditionActiveGrid]);
	const clickEventConditions = checkNavigationCondition([
		conditions.conditionActiveHamburger,
		conditions.conditionActiveGrid,
	]);

	if (event.target === gridRows) {
		return;
	}

	if (keyEventConditions || clickEventConditions) {
		toggleNavigation();
	}
}
...
function carousel() {
	...
	function getDot(currentSlideDot) {
		const carouselDotsCopy = [...carouselDots];
	...
	function moveSlider(slideNumber = undefined) {
		const allCarouselSectionsCopy = [...allCarouselSections];
